...
- В рамках одного счета не может быть больше 10 з/с
...
`barcode`       | STRING | YES | Уникальный идентификатор заказа. Генерируется на стороне клиента. Может представлять собой `Штриховой почтовый идентификатор`. Максимал размер 128 символов
...
`storage_period`| INT    | NO  | Срок хранения заказа в сутках
